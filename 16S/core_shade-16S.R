...
ps_hop<-readRDS("hop_pure_16.rds")
'%notin%' <- Negate('%in%')
ps_hop <- subset_samples(ps_hop, Week %notin% c(1,2,3,4,5)) # remove everything without enough samples to be reasonable
...
row.names(otu_table(ps_hop_cone))[row.names(otu_table(ps_hop_cone)) %in% row.names(otu_table(ps_hop_rhizome))] -> shared.bac # get taxa that are in both tissue types
...
# Can be set to the entire length of OTUs in the dataset, however it might take some time if more than 5000 OTUs are included.
...
coreis.tissue<-otu_ranked[1:lastCall,] # gets the core OTUs, but watch out
write.csv(coreis.tissue, file="corebytissue-updated.csv")
...
coreis.tissue %>% filter(otu %in% shared.bac) -> coreis.tissue
