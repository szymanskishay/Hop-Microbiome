...
adonis.whole.T<-adonis2(t(otu_ps_hop) ~ Tissue,  data=metadata_ps_hop, permutations=9999)
...
'%notin%'<- Negate('%in%')
ps_cone <- subset_samples(ps_cone, Week %notin% c(1,2,3,4,5)) # remove bad weeks
ps_cone <- subset_samples(ps_hop, Week)
...
# This gets the bacterial part of Table 1
