# Generates relative abundance plots (Figures 6, S1, S2, S5)
library(phyloseq)
library(dplyr)
library(data.table)
library(ggplot2)
...
Cone_abun_16s <- Cone_abun_16s[!Cone_abun_16s$Week %in% c(1, 2,5), ]

# -------- Cone: order by Week then Rep --------

Cone_abun_16s$Week <- factor(Cone_abun_16s$Week, levels = c(6, 7, 8, 9, 10))
...
# -------- Rhizome: order by Rep then Health (Diseased → Healthy within each Rep) --------

# set Health order (Diseased first)
rhizome_abun_16s$Health <- factor(rhizome_abun_16s$Health, 
                                  levels = c("Diseased", "Healthy"))

# ensure Rep is ordered numerically
rhizome_abun_16s$Rep <- factor(rhizome_abun_16s$Rep, 
                               levels = sort(unique(rhizome_abun_16s$Rep)))

# create ordered sample list (Rep first, then Health)
sample_order_dat2_16s <- rhizome_abun_16s %>%
  distinct(Sample, Rep, Health) %>%
  arrange(Rep, Health) %>%
  pull(Sample)

# apply ordering
rhizome_abun_16s$Sample <- factor(rhizome_abun_16s$Sample, 
                                  levels = sample_order_dat2_16s)

rhizome_abun_16s$Week <- factor(rhizome_abun_16s$Week)

combined_16s <- bind_rows(Cone_abun_16s, rhizome_abun_16s)
combined_16s <- combined_16s %>%
...
ps_hop.barplot_rank_3_16s= ggplot(combined_16s, aes(x = Sample, y = Abundance, fill = Rank_3)) + 
...
plot(ps_hop.barplot_rank_3_16s)
...
ps_hop.barplot.rank2= ggplot(combined_16s, aes(x = Sample, y = Abundance, fill = Rank_2)) + 
  facet_wrap(~Tissue, strip.position = "bottom", scales="free_x") +
